...
follow_user(FollowerID, FollowingID) ->
    ...
    ets:insert(?USER_TABLE_NAME, FollowerUser#user{following = sets:add_element(FollowingID, Following)}),
    ok.
...
save_tweet(ID, ParentId, Type, PosterID, Content) ->
    ...
    case re:run(Content, "@(\\S+)", RegexOptions) of
        {match, MentionList} -> 
            lists:foreach(fun ([_, Match]) -> 
                case ets:lookup(?MENTION_TABLE_NAME, Match) of
                    [{_, MentionSet}] -> ets:insert(?MENTION_TABLE_NAME, {Match, sets:add_element(ID, MentionSet)});
                    [] -> ets:insert(?MENTION_TABLE_NAME, {Match, sets:from_list([ID], [{version, 2}])})
                end
            end, MentionList);
        nomatch -> ok
    end,
        ...
        {match, HashtagList} -> 
            lists:foreach(fun ([_, Match]) -> 
                case ets:lookup(?HASHTAG_TABLE_NAME, Match) of
                    [{_, HashtagSet}] -> ets:insert(?HASHTAG_TABLE_NAME, {Match, sets:add_element(ID, HashtagSet)});
                    [] -> ets:insert(?HASHTAG_TABLE_NAME, {Match, sets:from_list([ID], [{version, 2}])})
                end
            end, HashtagList);
        nomatch -> ok
    end,
...
get_user_tweets(UserID, Seconds) ->
    ...
    ets:select(?TWEET_TABLE_NAME, [{#tweet{poster_id = UserID, created_at = '$1', _ = '_'}, [{'>', '$1', UTCBeforeSeconds}], ['$_']}]).
...
query_tweets(Query) ->
    case re:run(util:strip_whitespace(Query), "^(#(\\S+))$|^(@(\\S+))$|^(.+)$", [{capture, all, list}]) of
        {match, [_, _, Hashtag]} ->
